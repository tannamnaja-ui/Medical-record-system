    ...
    <div class="row g-1">
      ...
      <div class="col-6 col-md-3">
        ...
        <div class="info-value">
          ${p.cc
            ? `<span class="text-muted">${escHtml(p.cc)}</span>`
...
async function loadDoctorDiagnosis() {
  try {
    ...
    el.innerHTML = rows.map((row, idx) => {
      const diagText = escHtml(row.diag_text || '–');
      const doctorName = escHtml(row.doctor_name || '–');
      const diagDateTime = row.diag_datetime 
        ? String(row.diag_datetime).substring(0, 19).replace('T', ' ')
        : '–';
      
      return `<div style="border-bottom:1px solid #e0f0ff;color:#334155;">
        <div style="line-height:1.2;">
          <strong style="color:#1d4ed8;">วินิจฉัย:</strong> ${diagText}
        </div>
        <div style="font-size:14px;line-height:1.2;">
          <strong>แพทย์:</strong> ${doctorName}
        </div>
        <div style="font-size:14px;color:#666;line-height:1.2;">
          <strong>วันที่:</strong> ${diagDateTime}
        </div>
      </div>`;
    }).join('');
  } catch (e) {
